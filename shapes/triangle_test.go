...
func TestNormalOnATriangle(t *testing.T) {
	tri := NewTriangle(pm.Point(0, 1, 0), pm.Point(-1, 0, 0), pm.Point(1, 0, 0))

	n1 := tri.LocalNormalAt(pm.Point(0, 0.5, 0), nil, nil)
	n2 := tri.LocalNormalAt(pm.Point(-0.5, 0.75, 0), nil, nil)
	n3 := tri.LocalNormalAt(pm.Point(0.5, 0.25, 0), nil, nil)
...
func TestTriangleIntersection(t *testing.T) {
	...
	triangle := NewTriangle(pm.Point(0, 1, 0), pm.Point(-1, 0, 0), pm.Point(1, 0, 0))
